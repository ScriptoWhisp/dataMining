"""This code is taken from https://github.com/innarliiv/bigdata4digitalconstruction/blob/main/Script2_Draw_Building_TopDown_View.py"""
import json
...
def normilize_coords(polygon_coords):
    ...
    return [[x - min_x_rotated, y - min_y_rotated] for x, y in rotated_polygon_coords]


def gain_png(ehr, save_to):
    # Load polygon coordinates from JSON file
    polygon_coords = load_polygon_from_json(f'/PycharmProjects/dataMining/jsondata/{ehr}.ehr.json')

    transformed_rotated_coords = normilize_coords(polygon_coords)
    ...
    plt.title(f"Top-down view of the building {ehr}")
    plt.xlabel("X Coordinate")
    plt.ylabel("Y Coordinate")
    plt.grid(True)
    plt.axis('equal')
    plt.savefig(f"{save_to}/{ehr}")
    return transformed_rotated_coords
